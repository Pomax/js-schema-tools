...
export function makeChangeHandler(ignoreKey, filterKeyString) {
  const changeHandler = function changeHandler(object, operation) {
    ...
    if (type === `add` && !ignoreKey(key, type)) {
      ...
      // convert schema change to concrete data change
      const copy = { [key]: JSON.parse(JSON.stringify(value)) };
      fromSchemaToData(copy);
      level[propName] = copy[key];

      // TODO: FIXME: is this compatible with "non-schema" diffing?
