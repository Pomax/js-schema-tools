...
import { fromSchemaToData, validateField } from "../../models/model-store.js";
...
export function makeMigration(
  ...
  const rollbackChangeHandlers = operations
    .map((op) => {
      ...
      return `changeHandler.${op.rollback} = function (data, op) {\n  // Your rollback code goes here\n};\n`;
    })
    .join(`\n`);

  return `/**
  Autogenerated executable runner for migrating data based on the
  "${schema1.__meta.name}" schema from version ${version} to version ${version + 1}.
  ${
