...
class ModelRegistry {
  ...
  recordModelClass(BaseModel) {
    if (this.store) {
      // Do we have a store to work with, for loading/saving schema definitions?
      this.recordModelClassWithStoreBacking(BaseModel);
    } else {
      // If we don't have a store available, we can only record "this schema":
      this.REGISTER[BaseModel.name] = BaseModel.schema = new BaseModel();
    }
