...
import * as migrations from "../migrations/make-migration.js";
...
class ModelRegistry {
  ...
  recordModelClass(BaseModel) {
    ...
    modelSet.reverse().forEach((Model) => {
      ...
      if (stored) {
        console.log(`${modelName} stored`);

        const diffs = createDiff(stored, schema);

        console.log(`${modelName} has ${diffs.length} diffs`);
    ...
    console.log(`${modelName} difflist:`, diffList);
