...
class Models {
  ...
  static dataPath = false;

  /**
   * Used by save/load functions.
   */
  static setStoreLocation = function (dataPath) {
   ...
   */
  static create = function (Model, data = undefined) {
    const model = instantiateModel(Model);
   ...
   */
  static load = function (Model, recordName, storeLocation = undefined) {
    recordModelClass(Model);
    const dataPath = storeLocation ?? this.dataPath;
    const { name } = Model;
    ...
    if (recordName) {
      const filepath = `${dataPath}/${schema.__meta.name}/${recordName}.json`;
  ...
  static saveModel = function (
    ...
    const filepath = `${dataPath}/${schema.__meta.name}/${filename}.json`;
