...
export function fromSchemaToData(model) {
  ...
  props.forEach(([key, definition]) => {
    ...
    // non-model subtrees
      ...
      // If this is a proper model, we should already have its
      // associated schema stored in the registry.
      if (definition instanceof Model) {
        const modelName = definition.__proto__.constructor.name;
        schema = registry.REGISTER[modelName];
      }
