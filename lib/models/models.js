...
export class Models {
  ...
  static load = function (Model, recordName, storePath = Models.storePath) {
    ...
    if (recordName) {
      fileData = Store.loadRecord(schema, recordName);
    }
  ...
  static saveModel = function (instance) {
    ...
    const schema = MODEL_SCHEMA[name];
    const recordName = getRecordNameFor(schema, instance);
    Store.saveRecord(schema, instance, recordName);
  };
}

// ...
function getRecordNameFor(schema, instance) {
...
// Simple JSON-in-the-filesystem storage
const Store = {
  loadRecord: (schema, recordName) => {
    let fileData = undefined;
    const filepath = `${Models.storePath}/${schema.__meta.name}/${recordName}.json`;
  ...
  saveRecord: (schema, instance, recordName) => {
    const filepath = `${Models.storePath}/${schema.__meta.name}/${recordName}.json`;
    console.log(`saving ${recordName} -> ${filepath}`);
    fs.writeFileSync(filepath, instance.toString());
  },

  // TODO: move save code from model-store
  loadSchema: () => {},
  saveSchema: () => {},
};
