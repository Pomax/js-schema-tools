 ...
 */
export function instantiateModel(Model) {
 ...
 */
export function recordModelClass(Model) {
  const { name } = Model;
  ...
  if (!MODEL_SCHEMA[name]) {
    ...
    const stored = loadSchema(schema);
    ...
    if (stored) {
      ...
      if (diffs.length) {
        ...
        generateMigrationFile(stored, schema);
 ...
 */
function loadSchema(schema, dataPath = Models.dataPath) {
  const dir = `${dataPath}/${schema.__meta.name}/.schema/`;
 ...
 */
function generateMigrationFile(schema1, schema2) {
  ...
  let filepath = `${Models.dataPath}/${schema2.__meta.name}/${filename}`;
...
function saveSchema(Model) {
  ...
  const dataPath = Models.dataPath;
  schemaSet.forEach((entry) => {
    ...
    const dir = `${dataPath}/${__meta.name}/.schema`;
 ...
 */
function fromSchemaToData(model) {
