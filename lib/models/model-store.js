...
import {
  makeMigration,
  finalizeMigration,
} from "../schema/migrations/make-migration.js";
...
export function recordModelClass(Model, storePath = Models.storePath) {
  ...
  if (!MODEL_SCHEMA[name]) {
    ...
    } else saveSchema(Model, storePath);
...
function generateMigrationFile(schema1, schema2, storePath = Models.storePath) {
  ...
  const script = finalizeMigration(migration);
  fs.writeFileSync(filepath, script, `utf-8`);
  console.error(`Created schema migration runner [${filepath}]`);
}
...
function saveSchema(Model, storePath = Models.storePath) {
  ...
  schemaSet.forEach((entry) => {
    ...
    if (!fs.existsSync(newfilepath)) {
      ...
      console.log(`Created updated schema definition [${newfilepath}]`);
    }
  });
}
    ...
    // nested models need their own rewrite pass
      ...
      Object.defineProperty(model, key, {
        ...
        set: (data) => {
          ...
          else throw new Error(`Assignment violates property schema.`);
        }
      });
 ...
 *
 * TODO: we'll have to include this in the migration file, too
 */
