...
export function copyFromSource(source) {
  const target = {};
  setDataFrom(source, target);
  return target;
}

// deep-assign one object to another.
export function setDataFrom(source, target) {
...
export function removeModelDefaults(instance) {
  ...
  (function pruneTree(tree, schema) {
    Object.entries(tree).forEach(([key, value]) => {
      ...
      const required = definition.__meta?.required;
      if (!required && value === dflt) tree[key] = undefined;
    });
  })(pruned, schema);
