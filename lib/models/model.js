...
function testLevel(model, level, pathkey, term) {
  if (level[term] === undefined) {
    throw new Error(
      `${model.__proto__.constructor.name} model does not support ${pathkey}`
    );
  }
}
...
function getSetLevel(model, pathkey, value) {
  ...
  let level = model;
  const test = (term) => testLevel(model, level, pathkey, term);
